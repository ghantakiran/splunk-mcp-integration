#!/usr/bin/env python3
"""
Pydantic models for Word Export Service.

This module contains all data models used for API requests, responses,
and internal data structures for the Word export service.
"""
...
class ChartConfig(BaseModel):
    ...
    width: int = Field(default=400, ge=100, le=800, description="Chart width in points")
    height: int = Field(default=300, ge=100, le=600, description="Chart height in points")
...
class TableColumn(BaseModel):
    ...
    width: Optional[int] = Field(default=None, ge=50, le=500, description="Column width in points")
    alignment: str = Field(default="left", pattern="^(left|center|right)$", description="Text alignment")


class TableConfig(BaseModel):
    """Table configuration model."""
    columns: List[TableColumn] = Field(..., description="Table columns")
    style: str = Field(default="grid", description="Table style")
    header_style: Dict[str, Any] = Field(default_factory=dict, description="Header styling")
    row_style: Dict[str, Any] = Field(default_factory=dict, description="Row styling")
    border_style: Dict[str, Any] = Field(default_factory=dict, description="Border styling")
...
class Margin(BaseModel):
    """Document margin model."""
    top: float = Field(default=1.0, ge=0.5, le=3.0, description="Top margin in inches")
    bottom: float = Field(default=1.0, ge=0.5, le=3.0, description="Bottom margin in inches")
    left: float = Field(default=1.0, ge=0.5, le=3.0, description="Left margin in inches")
    right: float = Field(default=1.0, ge=0.5, le=3.0, description="Right margin in inches")


class PageSetup(BaseModel):
    """Page setup model."""
    orientation: PageOrientation = Field(default=PageOrientation.PORTRAIT, description="Page orientation")
    margins: Margin = Field(default_factory=Margin, description="Page margins")
    page_size: str = Field(default="A4", description="Page size")


class FontStyle(BaseModel):
    """Font style model."""
    family: FontFamily = Field(default=FontFamily.CALIBRI, description="Font family")
    size: int = Field(default=11, ge=8, le=72, description="Font size")
    bold: bool = Field(default=False, description="Bold text")
    italic: bool = Field(default=False, description="Italic text")
    color: str = Field(default="#000000", description="Font color")


class HeaderFooter(BaseModel):
    """Header/footer model."""
    text: str = Field(default="", description="Header/footer text")
    font_style: FontStyle = Field(default_factory=FontStyle, description="Font styling")
    alignment: str = Field(default="center", pattern="^(left|center|right)$", description="Text alignment")
    include_page_number: bool = Field(default=False, description="Include page numbers")
    include_date: bool = Field(default=False, description="Include current date")
...
class DocumentSection(BaseModel):
    ...
    content_type: str = Field(..., pattern="^(text|chart|table|page_break)$", description="Content type")
    ...
    style: Dict[str, Any] = Field(default_factory=dict, description="Section styling")


class DocumentLayout(BaseModel):
    """Document layout model."""
    sections: List[DocumentSection] = Field(..., description="Document sections")
    page_setup: PageSetup = Field(default_factory=PageSetup, description="Page setup")
    header: Optional[HeaderFooter] = Field(default=None, description="Document header")
    footer: Optional[HeaderFooter] = Field(default=None, description="Document footer")
    table_of_contents: bool = Field(default=False, description="Include table of contents")
    watermark: Optional[str] = Field(default=None, description="Watermark text")
...
class DocumentMetadata(BaseModel):
    ...
    comments: str = Field(default="", description="Document comments")


class DocumentConfig(BaseModel):
    """Document configuration model."""
    metadata: DocumentMetadata = Field(..., description="Document metadata")
    template: Template = Field(default=Template.PROFESSIONAL, description="Document template")
    layout: DocumentLayout = Field(..., description="Document layout")
    ...
    default_font: FontStyle = Field(default_factory=FontStyle, description="Default font style")
    color_scheme: ColorScheme = Field(default=ColorScheme.BLUE, description="Color scheme")
    branding: Dict[str, str] = Field(default_factory=dict, description="Custom branding")


# Request models
class WordExportRequest(BaseModel):
    """Word export request model."""
    job_name: str = Field(..., min_length=1, max_length=255, description="Job name")
    document_config: DocumentConfig = Field(..., description="Document configuration")
...
class BulkWordExportRequest(BaseModel):
    """Bulk Word export request model."""
    output_format: OutputFormat = Field(default=OutputFormat.DOCX, description="Output format")
    template: Template = Field(default=Template.PROFESSIONAL, description="Template")
    jobs: List[WordExportRequest] = Field(..., min_items=1, max_items=10, description="Export jobs")
...
class TemplateRequest(BaseModel):
    ...
    template_data: DocumentConfig = Field(..., description="Template data")
...
class AnalyticsResponse(BaseModel):
    ...
    avg_page_count: float = Field(..., description="Average page count")
...
class CapabilitiesResponse(BaseModel):
    ...
    max_document_pages: int = Field(..., description="Maximum document pages")
