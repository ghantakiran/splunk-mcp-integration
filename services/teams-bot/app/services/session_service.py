"""
Session service for managing Teams conversation sessions.
"""
...
from ..models.teams_models import TeamsSession, QueryResult

logger = get_logger(__name__)

class SessionService:
    """Service for managing Teams conversation sessions."""
    ...
    async def get_or_create_session(self, user_id: str, conversation_id: str) -> TeamsSession:
        """Get existing session or create a new one."""
        with LogContext(user_id=user_id, conversation_id=conversation_id):
            try:
                # Check for existing active session
                session = await self._get_active_session(user_id, conversation_id)
                ...
                async with self.db_pool.acquire() as conn:
                    await conn.execute(
                        """
                        INSERT INTO teams_sessions 
                        (id, user_id, conversation_id, started_at, last_activity, 
                ...
                logger.info("New Teams session created", session_id=session_id)
                return session
    ...
    async def update_session(self, session_id: str, query: str, response: Dict[str, Any]) -> bool:
        ...
        with LogContext(session_id=session_id):
            try:
                ...
                logger.info("Teams session updated", query_length=len(query))
                return True
    ...
    async def get_user_sessions(self, user_id: str, limit: int = 10) -> List[TeamsSession]:
        """Get recent sessions for a user."""
        try:
            async with self.db_pool.acquire() as conn:
                rows = await conn.fetch(
                    """
                    SELECT id, user_id, conversation_id, started_at, last_activity, 
                           history, context, preferences, is_active
                    FROM teams_sessions 
                    WHERE user_id = $1 
                ...
                for row in rows:
                    ...
                    sessions.append(TeamsSession(**session_data))
    ...
    async def get_conversation_sessions(self, conversation_id: str, limit: int = 5) -> List[TeamsSession]:
    ...
    async def close_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            # Remove from cache
            await self.redis_client.delete(f"teams_session:{session_id}")
            
            logger.info("Teams session closed", session_id=session_id)
            return True
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        try:
            ...
            async with self.db_pool.acquire() as conn:
                ...
                if updated_count > 0:
                    logger.info(f"Cleaned up {updated_count} expired Teams sessions")
    ...
    async def _get_active_session(self, user_id: str, conversation_id: str) -> Optional[TeamsSession]:
        """Get active session for user in conversation."""
        try:
            # Check cache first
            cache_key = f"active_teams_session:{user_id}:{conversation_id}"
            ...
            async with self.db_pool.acquire() as conn:
                row = await conn.fetchrow(
                    ...
                    LIMIT 1
                    """,
                    user_id, conversation_id, datetime.utcnow() - self.session_timeout
                )
                ...
                if row:
                    ...
                    session = TeamsSession(**session_data)
    ...
    async def _get_session_by_id(self, session_id: str) -> Optional[TeamsSession]:
        """Get session by ID from cache or database."""
        try:
            # Check cache first
            cache_key = f"teams_session:{session_id}"
            cached_data = await self.redis_client.get(cache_key)
            
            if cached_data:
                session_data = json.loads(cached_data)
                return TeamsSession(**session_data)
            ...
            async with self.db_pool.acquire() as conn:
                row = await conn.fetchrow(
                    ...
                    SELECT id, user_id, conversation_id, started_at, last_activity,
                           history, context, preferences, is_active
                    FROM teams_sessions WHERE id = $1
                    """,
                    session_id
                )
    ...
    async def _cache_session(self, session: TeamsSession):
        """Cache session data."""
        try:
            cache_key = f"teams_session:{session.id}"
            ...
            if session.is_active:
                active_key = f"active_teams_session:{session.user_id}:{session.conversation_id}"
