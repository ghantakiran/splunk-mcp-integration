...
class SplunkService:
    ...
    async def initialize(self):
        ...
        self.session = aiohttp.ClientSession(
            connector=connector,
            timeout=timeout,
            headers={"User-Agent": "Splunk-MCP-TeamsBot/1.0"}
        )
        ...
        try:
            payload = {
                ...
                "config": {
                    "width": 800,
                    "height": 400,
                    "theme": "teams",  # Teams-specific theme
                    "export_format": "png"
                },
                "user_preferences": user_context.get("preferences", {})
            }
    ...
    async def get_saved_searches(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.api_gateway_url}/api/v1/searches/saved",
                params=params,
                timeout=aiohttp.ClientTimeout(total=settings.api_gateway_timeout)
            ) as response:
                ...
                if response.status == 200:
                    result = await response.json()
                    return {"success": True, "searches": result.get("searches", [])}
                else:
