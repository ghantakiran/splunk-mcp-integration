#!/usr/bin/env python3
"""
CSV Export Service - Main Application

This service provides comprehensive CSV data export capabilities
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting CSV Export Service...")
    ...
    try:
        ...
        logger.info("CSV Export Service started successfully")
        yield
        ...
        # Cleanup
        logger.info("Shutting down CSV Export Service...")
        await close_db()
        await close_redis()
        logger.info("CSV Export Service shut down complete")


# Create FastAPI application
app = FastAPI(
    title="CSV Export Service",
    description="Advanced CSV data export service for Splunk MCP Integration",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def metrics():
    ...
    try:
        ...
        metrics_data = {
            "csv_export_queue_pending": queue_sizes.get("pending", 0),
            "csv_export_queue_processing": queue_sizes.get("processing", 0),
            "csv_export_queue_total": queue_sizes.get("total", 0),
            "csv_export_service_healthy": 1
        }
...
async def service_info():
    ...
    return {
        "service": "csv-export-service",
        "version": "1.0.0",
        "description": "Advanced CSV data export service",
        "features": [
        ...
        "supported_formats": ["csv", "tsv", "pipe-delimited", "custom"],
